...
- **续接分类**：`maybeContinueExistingTask()` 先走确定性与启发式规则，不足时才调用模型做 coarse-grained continuation classification；当 `confirm_required = true` 时，只允许文件、选区、错误等结构化补充证据续接到 `waiting_input / confirming_intent` 这类待用户补充的任务，不能续接正在执行的任务，也不能自动执行。
