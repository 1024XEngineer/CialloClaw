...
5. 调用 `attachMemoryReadPlans()` 把本轮记忆检索计划先挂到任务运行态，并把命中的摘要整理成后续执行可注入的记忆上下文；这些摘要只能作为历史参考，不能覆盖当前任务意图与系统约束。
...
#### 核心职责
- 在任务开始或确认后，通过 `attachMemoryReadPlans()` 预登记本轮记忆召回计划，并把命中的摘要挂到执行输入可消费的读取计划里，且明确标记为非指令性的历史参考；
