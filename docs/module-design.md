...
- `agent.notepad.convert_to_task` 以 `note_text / title` 作为正式任务文本输入；用户显式关联的路径型 `related_resources` 会写入 `TaskContextSnapshot.Files`，而系统派生的默认目录仍只保留为来源事项的展示与打开上下文，避免把 dashboard 展示兜底误判成用户执行输入。
