...
- `agent.notepad.convert_to_task` 以 `note_text / title` 作为正式任务文本输入，并复用普通 free-text 任务入口的确认门槛；用户显式填写的 `note_text` 优先进入正式输入，title-only 事项回退到原始 `title`，而不是使用 dashboard 展示态补写的说明文案；用户显式关联的路径型 `related_resources` 会写入 `TaskContextSnapshot.Files`，系统派生的默认目录仍只保留为来源事项的展示与打开上下文，避免把 dashboard 展示兜底误判成用户执行输入。
