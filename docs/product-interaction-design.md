...
* 悬浮球允许用户自由拖拽；拖拽过程中可临时越出当前屏幕可视区域，不自动贴边。只有当用户松开鼠标时，系统才按本次拖拽开始时所在屏幕的可用边缘执行回正与吸附：若悬浮球已接近该屏幕的可用边缘，才触发吸附与短时弹性停靠动画；若同时接近横向与纵向边缘，则应直接吸附到对应角落（左上 / 右上 / 左下 / 右下）。若松手位置未进入吸附阈值，则只回到该起始屏幕的可见范围内，不得自行贴边。左右贴边可继续保留较深的半隐藏效果；上下与四角贴边则不应简单复用“藏一半”的规则，顶部与上角停靠时需要保留脸部与冠羽可见，底部与下角停靠时也应至少保留脸部主体可见。进入贴边停靠后，在用户未悬停揭示悬浮球前，悬浮球整体视觉尺寸应收缩到原本的约 `0.7x`；当用户再次悬停并把悬浮球完整揭示出来时，悬浮球应恢复正常大小。只有在左侧或左侧角落的贴边停靠态且未揭示时，悬浮球才应做左右镜像；一旦完整揭示，就应恢复正常朝向。
...
* 路演阶段的主动推荐可以先由桌面端本地 mock 生成，单击悬浮球时综合当前窗口、选中文本、剪贴板、错误信号、停留时长和最近动作给出 1 条建议；推荐卡片只负责承接和外显，用户点中推荐后仍可进入正式 `task` pipeline。
* 为避免刷屏，同一上下文会保留短冷却，重复单击时优先聚焦已有推荐，而不是不断刷新相同建议。
