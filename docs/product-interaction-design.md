...
* 待机态左键单击应优先尝试展示 1 到 2 条基于当前上下文的轻量 recommendation；若当前没有可用 recommendation，则应把焦点交给下方轻量输入框，而不是继续保持无反馈状态。
...
* 待机态左键单击不属于“主动推荐自动触发”范畴；它是用户显式发起的轻接近动作，因此允许在单击后按需拉取 recommendation，或在没有 recommendation 时直接进入轻量输入承接。
...
* 反馈结构如下：
  ...
  * **悬浮球下方对话框**：作为轻量输入与控制区域，用于展示确认按钮；当用户点“修改意图”时，应临时借用同一个输入框输入自然语言修正，并在取消或提交后恢复原草稿

* 若用户点击确认，系统按照当前判断执行；若用户点“修改意图”，系统应保留当前草稿，临时把下方输入框切换为自然语言纠偏入口，并沿用同一会话继续补充说明。执行后的结果继续呈现在气泡中。
...
* 悬浮球下方对话框：提供确认按钮；当用户点“修改意图”时，临时借用同一个输入框输入自然语言修正
