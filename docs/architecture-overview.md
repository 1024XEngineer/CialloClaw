...
本文档定位为“架构设计文档”。它回答系统级分层、对象边界和主链路问题，但不替代协议真源、数据真源和模块实现细节。
...
| 类型                   | 代表对象                                                     | 架构作用                                                     |
| ---------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **对外正式对象**       | `Task`、`TaskStep`、`BubbleMessage`、`DeliveryResult`、`Artifact`、`ApprovalRequest`、`AuthorizationRecord`、`AuditRecord`、`RecoveryPoint`、`TodoItem`、`RecurringRule`、`MirrorReference` | 面向前端、协议和工作台的正式对象                             |
| **后端执行兼容对象**   | `Run`、`Step`、`Event`、`ToolCall`、`Citation`               | 用于执行、排障、回放和运行时事件表达                         |
| **治理与长期协作对象** | `MemorySummary`、`MemoryCandidate`、`RetrievalHit`、`TraceRecord`、`EvalSnapshot` | 服务记忆召回、镜像引用、质量评估与回放                       |
| **运行时协调结构**     | `TaskContextSnapshot`、`intent.Suggestion`、`runengine.TaskRecord`、持久化写入计划、恢复候选 | 仅用于当前实现中的编排、状态推进和持久化协同，不构成新增协议真源 |
...
推荐、巡检和事项默认都不直接进入任务主状态机。只有在用户接受、规则命中或明确升级动作发生后，才进入正式任务链。
...
- 架构设计文档只解释分层、边界和主链，不提前冻结协议字段和落表细节。
...
- `context.Service` 的稳定输出是 `TaskContextSnapshot`；其职责是把输入与现场事实归一化，而不是在接入层或执行层分散理解上下文。
- `intent.Service` 当前仍是轻量建议层；其产物是 `Suggestion`，不直接推进状态机，也不替代编排器。
- `runengine.TaskRecord` 是当前运行态核心桥接结构，用于连接对外 `task` 语义与内部 `run` 执行状态。
