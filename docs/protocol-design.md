...
- **系统处理**：将事项按 `note_text / title` 升级为正式任务，并复用普通 free-text 任务入口的确认门槛；用户显式填写的 `note_text` 优先进入正式任务文本输入，title-only 事项则回退到原始 `title`，不会把展示态补写文案当成正式执行输入；用户显式关联的路径型 `related_resources` 会进入正式任务文件上下文，系统派生的默认目录仍只保留为来源事项的展示与打开上下文
- **入参**：事项 ID、确认标记
