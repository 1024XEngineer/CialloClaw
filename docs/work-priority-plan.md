...
- [ ] 意图确认气泡能承接 `agent.task.confirm` 的确认、修正、补充说明，以及通过 `agent.task.control` 取消任务。
...
- [x] 结果页 / 浏览器交付形成真实承接，至少覆盖页面摘要、页面搜索、浏览器快照等场景。（后端已统一 `result_page.payload.url` 与 `agent.delivery.open.resolved_payload.url`；`page_read / page_search / browser_snapshot` 默认收口到正式结果页，dashboard / shell-ball 也已共享正式打开链路与结果页承接。）
...
- [x] Playwright 完整接入，已支持 `page_read / page_search / web_search / page_interact` 与健康检查回收。
- [x] Chromium 真实浏览器附着底座已接入，已支持 `browser_attach_current / browser_snapshot / browser_tabs_list` 的最小可用链路；其中 `browser_attach_current / browser_snapshot / browser_tabs_list` 默认按低风险浏览器观察能力处理，`browser_navigate / browser_tab_focus` 也已在公开 `http(s)` 目标或稳定标签页目标下放宽为默认可执行能力，显式 endpoint override 或敏感网页目标仍回到治理审批；`browser_interact` 继续保留审批边界，agent loop 当前默认开放 `browser_tabs_list / browser_navigate / browser_tab_focus`，但仍不开放 `browser_interact`。
- [x] Playwright 完整接入，已支持 `page_read / page_search / web_search / page_interact` 与健康检查回收；3b 已补齐执行层 attach hint 注入、attach-only snapshot 续跑保留和既有 `browser_*` intent 的主链路回归修复，但进程级 session narrowing 仍待后续扩展 attach contract 与 worker 目标选择逻辑。
...
- [ ] 补齐“意图确认气泡”这个原子功能：当系统进入意图确认态时，用户需要能在气泡里直接确认、取消任务、修正意图，而不是只看到一段说明文本。
