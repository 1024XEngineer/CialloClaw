...
- [x] Chromium 真实浏览器附着底座已接入，已支持 `browser_attach_current / browser_snapshot / browser_tabs_list` 的最小可用链路；`browser_navigate / browser_tab_focus / browser_interact` 已进入后端治理、审批边界和交付默认值，但 agent loop 仍只开放无需审批的浏览器能力。
