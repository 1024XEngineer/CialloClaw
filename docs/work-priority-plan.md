...
- [ ] 把 issue #261 中属于 4 号的屏幕感知主收口任务并回本分工文档，确保协议字段、Context/Intent、Orchestrator、Delivery、任务详情查询的口径和代码现状一致。
...
- [ ] `agent.input.submit` 与 `agent.task.start` 仍需统一承接“本轮屏幕感知引用”，确保前端不用依赖未登记的并行入口或裸平台对象就能触发视觉型任务。
...
- [ ] Orchestrator / Intent 仍需补齐“自然语言触发视觉型任务”的主链路承接，例如“帮我看看这个页面的报错”“看看当前屏幕上哪里出错了”应能落入正式 task，而不是要求前端先手工拼显式 intent。
...
- [ ] 屏幕感知相关失败和降级路径仍需继续冻结语义：授权拒绝、采样失败、OCR 失败、会话失效、无有效识别内容都应映射到既有错误段与既有 task/event/delivery 语义中，不新增视觉专项伪状态。
- [ ] 屏幕感知专项测试仍需继续覆盖协议字段、Context 归一化、视觉任务 intent 推断、授权后执行恢复、任务详情可回看证据链等关键路径，作为 4 号与 5 号协同联调的基础回归集。
