...
- [x] 错误信息承接的服务层封装已存在，并已在近场入口接入统一 UI 动作；主悬浮球单击会优先识别当前窗口错误信号，并允许用户直接走 `error_detected` 正式任务链路。
...
- [x] 补齐“错误信息承接”这个原子功能：当前窗口或当前页面出现报错时，用户可以直接把错误作为任务对象送入错误分析链，而不需要先手动改写成普通输入。
