...
- [x] 错误信息承接已通过近场统一 UI 动作进入正式入口。
- [ ] 意图确认气泡能承接确认动作，并支持用户通过轻量输入区对同一 `confirming_intent` 任务补充或修正自然语言；显式拒绝路径仍待补齐。
...
- [ ] 补齐“意图确认气泡”这个原子功能：当系统进入意图确认态时，用户需要能在气泡里直接确认，并能借用下方轻量输入区修正意图；显式取消 / 拒绝路径仍待补齐，而不是只看到一段说明文本。
