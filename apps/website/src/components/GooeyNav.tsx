...
const GooeyNav: React.FC<GooeyNavProps> = ({
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          try {
            ...
            // Ignore remove errors while the effect resets between quick clicks.
          }
        }, t);
      }, 30);
    }
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number, href: string) => {
    ...
    const navigationDelay = animationTime + Math.round(timeVariance / 2);
    ...
    <>
      ...
      <div className="relative" ref={containerRef}>
        <nav className="relative flex" style={{ transform: "translate3d(0,0,0.01px)" }}>
          <ul
            ref={navRef}
            className="relative z-[3] m-0 flex list-none gap-8 px-4 py-0"
            style={{
              color: "white",
          ...
          >
            {items.map((item, index) => (
              <li
                key={index}
                className={`relative cursor-pointer rounded-full text-white shadow-[0_0_0.5px_1.5px_transparent] transition-[background-color_color_box-shadow] duration-300 ease ${
