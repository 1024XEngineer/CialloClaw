...
type ShellBallMascotPetMode = "alert" | "happy" | "idle" | "listen" | "safe" | "think";

type ShellBallMascotPetState = {
  listenLocked: boolean;
  mode: ShellBallMascotPetMode;
};
...
/**
 * Keeps the legacy pet-state contract available for tests while the runtime
 * mascot uses the newer motion-driven renderer.
 */
export function getShellBallMascotPetState(input: {
  ...
  if (input.visualState === "confirming_intent" || input.selectionIndicatorVisible || input.hasAlertOpportunity) {
    return {
      listenLocked: false,
      mode: "alert",
    };
