...
/**
 * Loads one dashboard settings snapshot from JSON-RPC or from the local
 * baseline when mock mode is requested, then merges any scoped payload back
 * into the full local baseline.
 *
...
export async function loadDashboardSettingsSnapshot(
  ...
  if (source === "mock") {
    // Mock mode stays on the local baseline so roadshow pages do not need the
    // backend settings transport to render or refresh.
    return {
      settings: baseline,
      source: "mock",
      rpcContext: {
