...
function inferLocalUpdatedKeys(patch: DashboardSettingsPatch) {
...
export async function updateDashboardSettings(
  ...
  if (source === "mock") {
    // Roadshow mock mode keeps settings writes entirely local so the mirror
    // page can stay self-contained even when the backend is unavailable.
    persistPatchedSettings(patch);
    const refreshScope = inferDashboardSettingsRefreshScope(patch);
    const snapshot = await loadDashboardSettingsSnapshot("mock", refreshScope);

    return {
      snapshot,
      applyMode: "immediate",
      needRestart: false,
      updatedKeys: inferLocalUpdatedKeys(patch),
