...
const elements = {
  ...
  meetingPrompt: document.getElementById("meetingPrompt"),
  meetingPromptTitle: document.getElementById("meetingPromptTitle"),
  ...
  meetingWindowsBar: document.getElementById("meetingWindowsBar"),
  showSummaryWindowButton: document.getElementById("showSummaryWindowButton"),
  showCaptionsWindowButton: document.getElementById("showCaptionsWindowButton"),
...
function renderLayout() {
  ...
  elements.body.classList.toggle("meeting-candidate", Boolean(getMeetingDetectionState()?.promptVisible));
  elements.body.classList.toggle("dock-left", layout.dockSide === "left");
  elements.body.classList.toggle("dock-right", layout.dockSide === "right");
  elements.body.classList.toggle("dock-top", layout.dockSide === "top");
  elements.body.classList.toggle("dock-bottom", layout.dockSide === "bottom");
}
...
function renderMeetingPrompt() {
  ...
  elements.meetingPrompt.classList.toggle("hidden", !visible);
  if (!visible) {
    return;
  }

  elements.meetingPromptTitle.textContent = detection.candidateApp
    ? `检测到 ${detection.candidateApp}`
    : "检测到会议候选";
  elements.meetingPromptMeta.textContent = detection.note
    || "确认后会自动打开聊天窗口并开始会议总结。";
}

function renderMeetingWindowsBar() {
  const meeting = getMeetingState();
  const visible = Boolean(isRingOpen() && meeting?.enabled);
  elements.meetingWindowsBar.classList.toggle("hidden", !visible);
  if (!visible) {
    return;
  }

  elements.showSummaryWindowButton.disabled = Boolean(meeting.summaryOverlayVisible);
  elements.showCaptionsWindowButton.disabled = Boolean(meeting.captionsOverlayVisible);
  elements.showSummaryWindowButton.textContent = meeting.summaryOverlayVisible ? "概要窗已开" : "概要窗";
  elements.showCaptionsWindowButton.textContent = meeting.captionsOverlayVisible ? "字幕窗已开" : "字幕窗";
}
...
function bindEvents() {
  ...
  elements.meetingPromptAcceptButton.addEventListener("click", async (event) => {
    event.stopPropagation();
    markActivity();
    await api.acceptMeetingDetectionPrompt();
  });

  elements.meetingPromptDismissButton.addEventListener("click", async (event) => {
    event.stopPropagation();
    markActivity();
    await api.dismissMeetingDetectionPrompt();
  });

  elements.showSummaryWindowButton.addEventListener("click", (event) => {
    event.stopPropagation();
    markActivity();
    api.showMeetingSummaryOverlay();
  });

  elements.showCaptionsWindowButton.addEventListener("click", (event) => {
    event.stopPropagation();
    markActivity();
