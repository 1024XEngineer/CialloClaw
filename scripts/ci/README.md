...
so historical comment debt cannot silently return. In pull requests, CI passes
the base SHA so added-comment diagnostics still point at the PR diff. In local
mode, the added-comment guard evaluates staged hunks against the index snapshot
and unstaged hunks against the working tree, so partial staging in the same file
does not skew reported line numbers.
